...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('https://your-backend-url.com/login', { email, password })
      .then(result => {
        console.log(result);
        if (result.data.message === 'success') {
          ...
          alert(result.data.error); // Display error message to the user
        }
      })
      .catch(err => {
      ...
      <div className='d-flex vh-100 justify-content-center align-items-center bg-primary'>
        <div className='w-50 bg-white rounded p-4'>
          <form onSubmit={handleSubmit}>
            ...
            <Link
              to='/register'
              className='btn btn-default border w-100 bg-light text-decoration-none mt-2 rounded-0 w-100'
            >
              Register
            </Link>
          </form>
